...
class ImmutableReadonlyMap<K, V> implements ReadonlyMap<K, V> {
  private readonly backing: Map<K, V>;
...
export function indexTravelSnapshot(
  ...
  if (!candidate || !Array.isArray(candidate.edges)) {
    return invalidTravel("itinerary.travel.invalid", ["travel"]);
  }
  ...
  for (const edge of candidate.edges) {
    if (
      !edge ||
      typeof edge !== "object" ||
      typeof (edge as { fromPlaceId?: unknown }).fromPlaceId !== "string" ||
      typeof (edge as { toPlaceId?: unknown }).toPlaceId !== "string"
    ) {
      continue;
    }

    const typedEdge = edge as { fromPlaceId: string; toPlaceId: string };
    let destinations = destinationsByOrigin.get(typedEdge.fromPlaceId);
    if (!destinations) {
      destinations = new Set<string>();
      destinationsByOrigin.set(typedEdge.fromPlaceId, destinations);
    }
    if (destinations.has(typedEdge.toPlaceId)) {
      return invalidTravel("itinerary.travel.duplicate_edge", ["travel.edges"]);
    }
    destinations.add(typedEdge.toPlaceId);
