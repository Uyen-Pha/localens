...
export function canCancelBooking(row:ReviewedBooking,now:number){const match=bookingDepartures.find(x=>x.departure.id===row.departure_id);return row.status==='confirmed'&&!!match&&Date.parse(match.departure.startAt)-now>=48*60*60*1000;}
