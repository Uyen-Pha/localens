...
SELECT ok(has_function_privilege('localens_checkout_rpc_owner', 'private.record_checkout_audit_event(public.audit_event_type,uuid,public.audit_target_type,uuid,text,text,public.audit_metadata_key,text,numeric,boolean)', 'EXECUTE'), 'checkout owner can execute the identity-owned audit helper');
...
SELECT ok(
  ...
  AND (SELECT pg_get_expr(polqual, polrelid) FROM pg_catalog.pg_policy WHERE polname = 'bookings_projection_owner_select' AND polrelid = 'public.bookings'::regclass) ~* 'request\.jwt\.claim\.sub'
...
SELECT ok(
  has_column_privilege('localens_checkout_rpc_owner', 'private.checkout_idempotency', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.departures', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.tour_versions', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.tours', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.travel_snapshots', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.custom_requests', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.trip_plans', 'id', 'UPDATE')
  AND has_column_privilege('localens_checkout_rpc_owner', 'public.trip_plan_revisions', 'id', 'UPDATE')
  AND NOT has_table_privilege('localens_checkout_rpc_owner', 'private.checkout_idempotency', 'UPDATE')
  AND NOT has_table_privilege('localens_checkout_rpc_owner', 'public.departures', 'UPDATE')
  AND NOT has_table_privilege('localens_checkout_rpc_owner', 'public.trip_plans', 'UPDATE'),
  'checkout owner has only the id-column UPDATE grants required by row locks');
SELECT is(
  (SELECT count(*)::integer FROM pg_catalog.pg_policy
   WHERE polrelid = ANY(ARRAY[
     'public.departures'::regclass, 'public.tour_versions'::regclass, 'public.tours'::regclass,
     'public.travel_snapshots'::regclass, 'public.custom_requests'::regclass,
     'public.trip_plans'::regclass, 'public.trip_plan_revisions'::regclass
   ])
     AND polcmd = 'w'::"char"
     AND 'localens_checkout_rpc_owner'::regrole::oid = ANY(polroles)
     AND pg_get_expr(polqual, polrelid) = '(CURRENT_USER = ''localens_checkout_rpc_owner''::name)'
     AND pg_get_expr(polwithcheck, polrelid) = '(CURRENT_USER = ''localens_checkout_rpc_owner''::name)'),
  7,
  'checkout owner has explicit RLS policies for every locked source row');
...
SELECT ok(pg_get_functiondef('private.assert_checkout_transition()'::regprocedure) !~* '\(''accepted''::public\.quote_status, ''checkout_pending''::public\.quote_status\)', 'accepted quote never returns to checkout pending');
...
SELECT ok(
  ...
  AND pg_get_functiondef('private.record_checkout_session(uuid,uuid,text,timestamptz)'::regprocedure) ~* 'booking_row\.status.*confirmed.*payment_review'
...
-- denied above; the named view owner policy is JWT-subject-scoped, so an owner
...
SELECT ok(NOT EXISTS (
    ...
    WHERE granted.rolname IN ('localens_checkout_rpc_owner', 'localens_availability_rpc_owner', 'localens_booking_projection_owner')
      AND (member.rolname <> 'postgres' OR memberships.inherit_option)
  )
  AND (SELECT bool_and(pg_catalog.pg_has_role('postgres', role_name, 'SET')) FROM unnest(ARRAY['localens_checkout_rpc_owner', 'localens_availability_rpc_owner', 'localens_booking_projection_owner']) AS protected(role_name))
...
SELECT ok(EXISTS (SELECT 1 FROM pg_catalog.pg_enum AS e JOIN pg_catalog.pg_type AS t ON t.oid = e.enumtypid JOIN pg_catalog.pg_namespace AS n ON n.oid = t.typnamespace WHERE n.nspname = 'public' AND t.typname = 'booking_status' AND e.enumlabel = 'payment_review'), 'booking payment review state is available');
...
SELECT throws_ok($$SELECT * FROM private.start_checkout_tx('departure', '00000000-0000-0000-0000-000000000901'::uuid, 1, 'en'::public.locale, 'test', repeat('0', 64))$$::text, '42501'::character(5), NULL::text, 'unauthenticated checkout is rejected'::text);
SELECT throws_ok($$SELECT * FROM private.record_checkout_session('00000000-0000-0000-0000-000000000901'::uuid, '00000000-0000-0000-0000-000000000902'::uuid, 'cs_test_localens_001', now() + interval '30 minutes')$$::text, '42501'::character(5), NULL::text, 'unauthenticated session recording is rejected'::text);
SELECT throws_ok($$SELECT * FROM private.compensate_checkout_failure('00000000-0000-0000-0000-000000000902'::uuid)$$::text, '42501'::character(5), NULL::text, 'unauthenticated compensation is rejected'::text);
...
SELECT ok(pg_get_function_result('public.get_live_departure_availability()'::regprocedure) !~* 'owner_user_id|hold_id|booking_id', 'availability leaks no customer or hold identity');
...
SELECT ok(NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc WHERE prokind = 'f' AND pg_get_functiondef(oid) ~* 'raw_guest_token|stripe_signature|service_role_key'), 'checkout SQL stores no raw secrets');
