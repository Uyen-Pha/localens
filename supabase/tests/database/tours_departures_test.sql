...
GRANT EXECUTE ON FUNCTION private.valid_tour_copy_array(text[]), private.assert_published_tour_complete(uuid) TO postgres;
RESET ROLE;
...
SELECT ok(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tour_version_stops'::regclass AND conname = 'tour_version_stops_pkey' AND contype = 'p'), 'stop positions are unique per version');
...
SELECT ok(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tour_versions'::regclass AND conname = 'tour_versions_price_vnd_per_person_check' AND contype = 'c'), 'version money uses the safe engine bound');
SELECT ok(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tour_versions'::regclass AND conname = 'tour_versions_duration_minutes_check' AND contype = 'c'), 'version duration is bounded');
...
SELECT ok(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.departures'::regclass AND conname = 'departures_tour_version_id_fkey' AND contype = 'f'), 'departures reference versions');
...
SELECT ok(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tour_version_stops'::regclass AND conname = 'tour_version_stops_position_check' AND contype = 'c'), 'stop positions are mapper bounded');
...
SELECT is((SELECT count(*)::integer FROM pg_policy WHERE polrelid = ANY(ARRAY['public.tours'::regclass, 'public.tour_versions'::regclass]) AND polcmd = 'w'::"char" AND 'localens_tour_guard_owner'::regrole::oid = ANY(polroles) AND pg_get_expr(polqual, polrelid) = 'true' AND pg_get_expr(polwithcheck, polrelid) = 'true'), 2, 'tour lifecycle exposes exactly two guard row-lock policies');
...
SELECT ok(
  to_regprocedure('private.assert_published_tour_complete(uuid)') IS NOT NULL
  AND EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'public.tour_versions'::regclass AND conname = 'tour_versions_source_url_authority_check' AND contype = 'c'),
  'publication path enforces mapper-compatible URL authority');
SELECT ok(NOT has_column_privilege('localens_tour_guard_owner', 'public.tours', 'status', 'UPDATE')
  AND NOT has_column_privilege('localens_tour_guard_owner', 'public.tour_versions', 'status', 'UPDATE'), 'tour guard row-lock policy cannot mutate lifecycle status');
...
SELECT throws_ok($$UPDATE public.tours SET status = 'published' WHERE id = '00000000-0000-0000-0000-000000000920'::uuid$$::text, '23514'::character(5), NULL::text, 'incomplete tour cannot publish'::text);
...
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'published', published_at = now() WHERE id = '00000000-0000-0000-0000-000000000922'::uuid$$::text, '23514'::character(5), NULL::text, 'incomplete version cannot publish'::text);
INSERT INTO public.tour_version_translations (tour_version_id, locale, title, summary, meeting_point)
...
SELECT throws_ok($$UPDATE public.tours SET status = 'draft' WHERE id = '00000000-0000-0000-0000-000000000921'::uuid$$::text, '42501'::character(5), NULL::text, 'published tour cannot return to draft'::text);
...
SELECT throws_ok($$UPDATE public.tour_translations SET tour_id = '00000000-0000-0000-0000-000000000926'::uuid WHERE tour_id = '00000000-0000-0000-0000-000000000921'::uuid AND locale = 'en'$$::text, '23514'::character(5), NULL::text, 'current translation cannot leave a published tour'::text);
...
RESET ROLE;
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'retired' WHERE id = '00000000-0000-0000-0000-000000000927'::uuid$$::text, '42501'::character(5), NULL::text, 'last published version cannot retire while parent remains non-archived'::text);
...
SELECT throws_ok($$UPDATE public.tours SET status = 'published' WHERE id = '00000000-0000-0000-0000-000000000926'::uuid$$::text, '42501'::character(5), NULL::text, 'archived tour cannot be published'::text);
SET LOCAL ROLE anon;
...
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'published', published_at = now() WHERE id = '00000000-0000-0000-0000-000000000928'::uuid$$::text, '23514'::character(5), NULL::text, 'non-canonical catalog stop slug blocks publication'::text);
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'published', published_at = now() WHERE id = '00000000-0000-0000-0000-000000000929'::uuid$$::text, '23514'::character(5), NULL::text, 'non-canonical catalog stop title blocks publication'::text);
...
SELECT throws_ok($$UPDATE public.tour_versions SET price_vnd_per_person = 100001 WHERE id = '00000000-0000-0000-0000-000000000922'::uuid$$::text, '42501'::character(5), NULL::text, 'published version price is immutable'::text);
SELECT throws_ok($$DELETE FROM public.tour_versions WHERE id = '00000000-0000-0000-0000-000000000922'::uuid$$::text, '42501'::character(5), NULL::text, 'published version delete is rejected'::text);
SELECT throws_ok($$UPDATE public.tour_version_translations SET title = 'mutated' WHERE tour_version_id = '00000000-0000-0000-0000-000000000922'::uuid AND locale = 'en'$$::text, '42501'::character(5), NULL::text, 'published version translation update is rejected'::text);
SELECT throws_ok($$DELETE FROM public.tour_version_stops WHERE tour_version_id = '00000000-0000-0000-0000-000000000922'::uuid AND position = 1$$::text, '42501'::character(5), NULL::text, 'published version stop delete is rejected'::text);
INSERT INTO public.tour_versions (
  id, tour_id, catalog_snapshot_id, status, duration_minutes,
  price_vnd_per_person, inclusions, exclusions, cancellation_policy,
  source_url, verified_at, attribution, license
...
SELECT throws_ok($$INSERT INTO public.tour_version_stops (tour_version_id, catalog_snapshot_id, position, place_id) VALUES ('00000000-0000-0000-0000-000000000923'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 1, '00000000-0000-0000-0000-000000000999'::uuid)$$::text, '23503'::character(5), NULL::text, 'stop cannot reference a place outside its catalog snapshot'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY[' guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour copy arrays reject untrimmed values'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://user@example.invalid/fixture', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects authority credentials'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid:bad/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects malformed ports'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid:443/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects explicit ports'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid:65536/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects out-of-range ports'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://user%40example.invalid/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects encoded authority credentials'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://999.999.999.999/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects numeric IPv4-looking authority'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://999.999/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects numeric two-label authority'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://xn--/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects invalid punycode authority'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://0x100000000/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects hexadecimal numeric authority'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://xn--.example/path', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tour source URL rejects punycode marker before valid TLD'::text);
...
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY(SELECT 'item-' || n FROM generate_series(1, 33) AS s(n)), ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, '33 inclusions are rejected'::text);
SELECT throws_ok($$INSERT INTO public.tour_version_stops (tour_version_id, catalog_snapshot_id, position, place_id) VALUES ('00000000-0000-0000-0000-000000000903'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 65, '00000000-0000-0000-0000-000000000912'::uuid)$$::text, '23514'::character(5), NULL::text, 'stop position 65 is rejected'::text);
SELECT throws_ok($$INSERT INTO public.tours (slug, status) VALUES (repeat('a', 161), 'draft')$$::text, '23514'::character(5), NULL::text, '161-character tour slug is rejected'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture#fragment', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'source URL fragments are rejected'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture?UTM_source=campaign', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'tracking query keys are case-insensitively rejected'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture?full_name=Person', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'sensitive query keys are rejected'::text);
SELECT throws_ok($$INSERT INTO public.tour_versions (tour_id, catalog_snapshot_id, status, duration_minutes, price_vnd_per_person, inclusions, exclusions, cancellation_policy, source_url, verified_at, attribution, license) VALUES ('00000000-0000-0000-0000-000000000902'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 'draft', 120, 100000, ARRAY['guide'], ARRAY['transfer'], 'No refunds.', 'https://example.invalid/fixture?%66ull_name=Person', DATE '2026-08-20', 'Fixture', 'CC BY 4.0')$$::text, '23514'::character(5), NULL::text, 'percent-encoded query keys are rejected'::text);
...
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'retired' WHERE id = '00000000-0000-0000-0000-000000000922'::uuid$$::text, '42501'::character(5), NULL::text, 'scheduled departure blocks version retirement'::text);
SELECT throws_ok($$INSERT INTO public.departures (tour_version_id, start_at, end_at, status, capacity) VALUES ('00000000-0000-0000-0000-000000000903'::uuid, TIMESTAMPTZ '2026-09-01 08:00:00+07', TIMESTAMPTZ '2026-09-01 10:00:00+07', 'scheduled', 12)$$::text, '42501'::character(5), NULL::text, 'scheduled departure cannot use a draft version'::text);
SELECT throws_ok($$INSERT INTO public.departures (tour_version_id, start_at, end_at, status, capacity) VALUES ('00000000-0000-0000-0000-000000000903'::uuid, TIMESTAMPTZ '2026-09-01 08:00:00+07', TIMESTAMPTZ '2026-09-01 10:00:00+07', 'cancelled', 12)$$::text, '42501'::character(5), NULL::text, 'departure must start in scheduled state'::text);
...
SELECT throws_ok($$UPDATE public.tour_versions SET status = 'retired' WHERE id = '00000000-0000-0000-0000-000000000922'::uuid$$::text, '42501'::character(5), NULL::text, 'sold-out departure blocks version retirement'::text);
SELECT throws_ok($$UPDATE public.tours SET status = 'archived' WHERE id = '00000000-0000-0000-0000-000000000921'::uuid$$::text, '42501'::character(5), NULL::text, 'active departure blocks parent tour archive'::text);
SELECT throws_ok($$UPDATE public.departures SET capacity = 13 WHERE id = '00000000-0000-0000-0000-000000000904'::uuid$$::text, '42501'::character(5), NULL::text, 'departure capacity is immutable after creation'::text);
SELECT throws_ok($$INSERT INTO public.departures (tour_version_id, start_at, end_at, status, capacity) VALUES ('00000000-0000-0000-0000-000000000922'::uuid, TIMESTAMPTZ '2026-09-02 08:00:00+07', TIMESTAMPTZ '2026-09-02 10:00:00+07', 'scheduled', 0)$$::text, '23514'::character(5), NULL::text, 'zero-capacity departure is rejected'::text);
SELECT throws_ok($$INSERT INTO public.departures (tour_version_id, start_at, end_at, status, capacity) VALUES ('00000000-0000-0000-0000-000000000922'::uuid, TIMESTAMPTZ '2026-09-02 08:00:00+07', TIMESTAMPTZ '2026-09-02 08:00:00+07', 'scheduled', 10)$$::text, '23514'::character(5), NULL::text, 'zero-length departure is rejected'::text);
...
SELECT throws_ok($$UPDATE public.departures SET status = 'completed' WHERE id = '00000000-0000-0000-0000-000000000905'::uuid$$::text, '42501'::character(5), NULL::text, 'cancelled departure cannot complete'::text);
...
SELECT throws_ok($$INSERT INTO public.tour_version_stops (tour_version_id, catalog_snapshot_id, position, place_id) VALUES ('00000000-0000-0000-0000-000000000922'::uuid, '00000000-0000-0000-0000-000000000901'::uuid, 2, '00000000-0000-0000-0000-000000000912'::uuid)$$::text, '42501'::character(5), NULL::text, 'retired version stops cannot be inserted'::text);
SET LOCAL ROLE anon;
